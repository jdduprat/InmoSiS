...
$PBExportComments$%SPANISH% PFC DataWindow QueryMode service
forward
...
return of_Load (as_title, as_pathname, as_filename, ".txt", "Query (*.txt),*.txt,Todos los archivos (*.*),*.*") 
...
return of_Load (as_title, as_pathname, as_filename, as_ext, "Query (*.txt),*.txt,Todos los archivos (*.*),*.*") 
end function
...
anv_infoattrib.is_description = 'Proporciona funcionalidad de QueryMode.'

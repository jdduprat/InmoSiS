...
$PBExportComments$%SPANISH% PFC DataStore class
forward
...
//////////////////////////////////////////////////////////////////////////////
//
//	Copyright $$HEX2$$a9002000$$ENDHEX$$1996-2001 Sybase, Inc. and its subsidiaries.  All rights reserved.
//	Any distribution of the PowerBuilder Foundation Classes (PFC)
//	source code by other than Sybase, Inc. and its subsidiaries is prohibited.
//
//////////////////////////////////////////////////////////////////////////////
...
If li_rc < 0 Then 
	...
	If Not lb_skipmessage Then
		...
		Else
			ls_message = "Fall$$HEX2$$f3002000$$ENDHEX$$la validaci$$HEX1$$f300$$ENDHEX$$n de AcceptText en DataStore.  "+&
							 "Columna "+ ls_msgparm[1] + &
							 " en la fila "+ ls_msgparm[2] + "."
	...
	If Not lb_skipmessage Then
		...
		Else
			of_MessageBox ("pfc_checkrequired_missingvalue", gnv_app.iapp_object.DisplayName, &
				"Falta un valor de DataStore necesario en " + as_colname + " en la fila "  + &
...
If IsValid(gnv_app.inv_debug) Then				
	of_MessageBox ("pfc_getparentwindow_debug", "Biblioteca PowerBuilder Classes", &
		"Para determinadas funciones, " + &
		"El DataStore de PFC necesita una referencia a su ventana padre. " + &
		"Se ha encontrado uno de estos casos. Para que el datastore "+&
		"sepa cu$$HEX1$$e100$$ENDHEX$$l es su ventana padre, llame a la funci$$HEX1$$f300$$ENDHEX$$n of_SetParentWindow(...) "+&
		"despu$$HEX1$$e900$$ENDHEX$$s de la creaci$$HEX1$$f300$$ENDHEX$$n del DataStore.", &
		Exclamation!, Ok!, 1)
End If
...
// There is no valid parent to return.
If IsValid(gnv_app.inv_debug) Then				
	of_MessageBox ("pfc_getparent_debug", "Biblioteca PowerBuilder Classes", &
		"Para determinadas funciones, " + &
		"El DataStore de PFC necesita una referencia a su objeto padre. " + &
		"Se ha encontrado uno de estos casos. Para que el datastore "+&
		"sepa cu$$HEX1$$e100$$ENDHEX$$l es su ventana padre, llame a la funci$$HEX1$$f300$$ENDHEX$$n of_SetParent(...) "+&
		"despu$$HEX1$$e900$$ENDHEX$$s de la creaci$$HEX1$$f300$$ENDHEX$$n del DataStore.", &
		Exclamation!, Ok!, 1)
End If
		...
		Else
			...
			If ls_sqlsyntax <> sqlsyntax Then
				...
				Else
					...
					of_Messagebox ('pfc_sqlpreview_error', 'SQLSpy en SQLPreview', &
						'Se ha encontrado un error con la siguiente instrucci$$HEX1$$f300$$ENDHEX$$n SQL:~r~n~r~n'+ &
...
ls_message = "Ha ocurrido un error de la base de datos.~r~n~r~n~r~n" + &
	"C$$HEX1$$f300$$ENDHEX$$digo de error de la base de datos:  " + String (sqldbcode) + "~r~n~r~n" + &
	"Mensaje de error de la base de datos:~r~n" + sqlerrtext
...
If IsValid(gnv_app.inv_debug) Then
	If IsValid(gnv_app.inv_debug.inv_sqlspy) Then
		...
		ls_sqlspymessage = "C$$HEX1$$f300$$ENDHEX$$digo de error de la base de datos:  " + String (sqldbcode) + "~r~n" + &
			"Mensaje de error de la base de datos:  " + sqlerrtext
...
If IsValid(gnv_app.inv_debug) Then
	If IsValid(gnv_app.inv_debug.inv_sqlspy) Then
		...
		ls_sqlspymessage = "Filas recuperadas = "+ string(rowcount)

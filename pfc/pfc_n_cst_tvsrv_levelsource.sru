...
$PBExportComments$%SPANISH% PFC Treeview Levelsource Service
forward
...
Constant String UNDO_INSERT = "Insertar"
Constant String UNDO_DELETE = "Eliminar"
...
if al_amount = 1 then
	return of_MessageBox ("pfc_tv_datasource_confirmdeletesingle", "Confirmar eliminaci$$HEX1$$f300$$ENDHEX$$n", &
			"$$HEX1$$bf00$$ENDHEX$$Seguro que desea eliminar " + is_displayitem + "?", question!, yesno!, 1)
else
	return of_MessageBox ("pfc_tv_datasource_confirmdeletemulti", "Confirmar m$$HEX1$$fa00$$ENDHEX$$ltiple eliminaci$$HEX1$$f300$$ENDHEX$$n", &
			"$$HEX1$$bf00$$ENDHEX$$Seguro que desea eliminar estos " + &
...
If IsValid(gnv_app.inv_debug) Then
	If IsValid(gnv_app.inv_debug.inv_sqlspy) Then
		...
		ls_sqlspymessage = 'Actualizaci$$HEX1$$f300$$ENDHEX$$n inteligente - '
		...
		End Choose
		If ab_insert Then
			ls_sqlspymessage = ls_sqlspymessage + 'Insertar '
		End If
		If ab_update Then
			ls_sqlspymessage = ls_sqlspymessage + 'Actualizaci$$HEX1$$f300$$ENDHEX$$n '
		End If
		If ab_delete Then
			ls_sqlspymessage = ls_sqlspymessage + 'Eliminar '
		End If

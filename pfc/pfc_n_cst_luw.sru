...
$PBExportComments$%SPANISH% PFC Logical Unit of Work service
forward
...
// If requested use the assist funtionality.
If ib_assist Then 
	of_Assist(is_instancename+'.of_SetDBErrorMsg a='+anv_dberrorattrib.is_errormsg)
End If
...
// Display the message.
of_Messagebox ('pfc_luw_dberror','Guardar', anv_dberrorattrib.is_errormsg, &
	StopSign!, Ok!, 1)	
...
// If requested use the assist funtionality.
If ib_assist Then 
	of_Assist(is_instancename+'.of_dberror() mostr$$HEX2$$f3002000$$ENDHEX$$el error.')
End If
...
If ai_saverc > 0 Then
	ls_msgheader = 'Se produjo un error de transacci$$HEX1$$f300$$ENDHEX$$n de base de datos.~r~n~r~n'+&
		'La funci$$HEX1$$f300$$ENDHEX$$n LUW.of_EndTran() deb$$HEX1$$ed00$$ENDHEX$$a guardar (Commit) todas las transacciones.~r~n'+ &
		'A continuaci$$HEX1$$f300$$ENDHEX$$n se detalla el problema.~r~n~r~n'
	ls_msgmainerror = 'La instrucci$$HEX1$$f300$$ENDHEX$$n de transacci$$HEX1$$f300$$ENDHEX$$n que ha iniciado el error - '+ &
		atr_secondary[1].ClassName()+'('+atr_secondary[1].of_GetName()+').of_Commit()' + &
		'~r~n~r~n'
	If li_maxperformed > 0 Then
		ls_msgperformedheader = 'Con antelaci$$HEX1$$f300$$ENDHEX$$n al error, se guardaron (Commit) las siguientes transacciones:~r~n'
	End If
	ls_msgsecondaryheader = 'Despu$$HEX1$$e900$$ENDHEX$$s del error, se ha ejecutado lo siguiente:~r~n'
Else
	ls_msgheader = 'Se produjo un error de transacci$$HEX1$$f300$$ENDHEX$$n de base de datos.~r~n~r~n'+&
		'La funci$$HEX1$$f300$$ENDHEX$$n LUW.of_EndTran() deb$$HEX1$$ed00$$ENDHEX$$a ejecutar operaciones RollBack en todas las transacciones.~r~n'+ &
		'A continuaci$$HEX1$$f300$$ENDHEX$$n se detalla el problema.~r~n~r~n'
	ls_msgmainerror = 'La instrucci$$HEX1$$f300$$ENDHEX$$n de transacci$$HEX1$$f300$$ENDHEX$$n que ha iniciado el error - '+ &
		atr_secondary[1].ClassName()+'('+atr_secondary[1].of_GetName()+').of_Rollback()' + &
		'~r~n~r~n'		
	If li_maxperformed > 0 Then
		ls_msgperformedheader = 'Con antelaci$$HEX1$$f300$$ENDHEX$$n al error, se realiz$$HEX2$$f3002000$$ENDHEX$$un RollBack de las siguientes transacciones:~r~n'
	End If
	ls_msgsecondaryheader = 'Despu$$HEX1$$e900$$ENDHEX$$s del error, se ha ejecutado lo siguiente:~r~n'
End If
...
For li_idx = 1 to li_maxsecondary
	...
	ls_msgsecondary += atr_secondary[li_idx].ClassName()+ &
		...
		' c$$HEX1$$f300$$ENDHEX$$digo de retorno= '+string(ai_secondaryrc[li_idx])
Next
...
If li_maxperformed + li_maxsecondary >= 2 Then
	of_Messagebox('pfc_luw_endtranerror','Guardar',ls_msg,StopSign!,Ok!,1)
End If
...
For li_i = 1 to li_max
	If atr_control[li_i].of_Begin() < 0 Then
		...
		If ib_assist Then
			...
			of_assist(is_instancename+'.of_BeginTran() fall$$HEX2$$f3002000$$ENDHEX$$con '+ls_name+' of_Begin()')
		End If		
		Return -1
	End If
Next
...
// If requested use the assist funtionality.
If ib_assist Then
	of_assist(is_instancename+'.of_BeginTran() ejecut$$HEX2$$f3002000$$ENDHEX$$satisfactoriamente of_Begin()(s).')
End If
...
For li_i = 1 to li_max
	If ai_saverc > 0 Then
		If atr_control[li_i].of_Commit() < 0 Then
			...
			// If requested use the assist funtionality.
			If ib_assist Then
				of_assist(is_instancename+'.of_EndTran() fall$$HEX2$$f3002000$$ENDHEX$$of_Commit()(s).')
			End If						
			Return -1
		End If
	Else
		If atr_control[li_i].of_Rollback() < 0 Then
			...
			// If requested use the assist funtionality.
			If ib_assist Then
				of_assist(is_instancename+'.of_EndTran() fall$$HEX2$$f3002000$$ENDHEX$$of_RollBack()(s).')
			End If			
			Return -1
		End If		
	End If
...
If ib_assist Then
	If ai_saverc > 0 Then
		of_assist(is_instancename+'.of_EndTran() ejecut$$HEX2$$f3002000$$ENDHEX$$satisfactoriamente of_Commit()(s).')
	Else
		of_assist(is_instancename+'.of_EndTran() ejecut$$HEX2$$f3002000$$ENDHEX$$satisfactoriamente of_Rollback()(s).')
	End If
End If
	...
	ElseIf TypeOf (lpo_tocheck) = DataStore! Then
		...
		If lds_nonpfc.RowCount() + lds_nonpfc.FilteredCount() + &
			...
			If la_rc < 0 Then
				...
				ls_errmessage = "Fall$$HEX2$$f3002000$$ENDHEX$$la validaci$$HEX1$$f300$$ENDHEX$$n de AcceptText en DataStore.  "+&
							 "Columna "+ lds_nonpfc.GetColumnName() + &
							 " en la fila "+ string(lds_nonpfc.GetRow()) + "."

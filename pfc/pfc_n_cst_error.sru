...
$PBExportComments$%SPANISH% PFC Message Service class
forward
...
string		is_notifypretitle = 'Aviso de correo - '
...
If li_replacecount <> li_parmscount Then
	...
	ls_stringparms = '~r~n ~r~n' + 'Argumentos del mensaje:'
...
If Len(ls_date) > 0 Then
	// Optional datetime.
	as_text = 'Fechahora: '+String(inv_errorpass.idt_date) + NEWLINE + NEWLINE
End If

// Required values.
as_text +=  'T$$HEX1$$ed00$$ENDHEX$$tulo:    '+inv_errorpass.is_title + NEWLINE + NEWLINE + &
				'Mensaje: '+inv_errorpass.is_text + NEWLINE + NEWLINE

If Len(inv_errorpass.is_user) > 0 Then
	// Optional user value.
	as_text += 'Usuario:     '+ inv_errorpass.is_user + NEWLINE
End If

If Len(inv_errorpass.is_usertext) >0 Then
	// Optional comments.
	as_text += 'Comentarios: '+inv_errorpass.is_usertext
End If
...
CHOOSE CASE ii_logfilestyle
	CASE TAB_DELIMITED
		as_text=	"Fechahora: "+ ls_date + TAB + &
					"T$$HEX1$$ed00$$ENDHEX$$tulo:    "+ inv_errorpass.is_title + TAB + &
					"Mensaje: "+ inv_errorpass.is_text + TAB + &
					"Usuario:     "+ inv_errorpass.is_user + TAB + &
					"Comentarios: "+ inv_errorpass.is_usertext
	
	CASE NEWLINE_DELIMITED
		as_text=	"Fechahora: "+ ls_date + NEWLINE + &
					"T$$HEX1$$ed00$$ENDHEX$$tulo:    "+ inv_errorpass.is_title + NEWLINE + &
					"Mensaje: "+ inv_errorpass.is_text + NEWLINE + &
					"Usuario:     "+ inv_errorpass.is_user + NEWLINE + &
					"Comentarios: "+ inv_errorpass.is_usertext + NEWLINE	
...
If Len(Trim(as_msgid)) >0 Then
	If IsNull(ids_messages) Or Not IsValid(ids_messages) Then
		...
		inv_errorpass.is_text = 'Mensaje predefinido "'+as_msgid+'" no se ha encontrado.~r~n ~r~n' +&
										'Los mensajes predefinidos no encontrados.'
	Else
		...
		Else
			// Entry was not found.
			inv_errorpass.is_text = 'Mensaje predefinido "'+as_msgid+'" No se encuentra.'
		End If

...
$PBExportComments$%SPANISH% PFC DataWindow Linkage service
forward
	...
	If of_ConfirmOnDelete() Then
		...
		Else
			li_option = of_MessageBox('pfc_linkage_deleterow', &
							gnv_app.iapp_object.DisplayName, &
							'$$HEX1$$bf00$$ENDHEX$$Eliminar filas', Exclamation!, OkCancel!, 1)
		End If
		If li_option <> 1 Then
...
anv_infoattrib.is_description = &
	'Servicio para vincular acciones de DataWindow. Normalmente se suele utilizar para '+&
	'procesos maestro/detalle pero puede utilizarse para otros fines. Este objeto '+&
	'incluye funciones que ayudan a coordinar las funciones t$$HEX1$$ed00$$ENDHEX$$picas de DataWindow '+&
	'grupos de DataWindows relacionados.'
	...
	If al_row = ll_originalrow Then
		...
		Else
			of_MessageBox ('pfc_linkage_checkrequiredmissing', gnv_app.iapp_object.DisplayName, &
				"Falta el valor requerido para " + &
				as_colname + " en la fila "  + String (al_row) + &
				". Introduzca un valor.", information!, Ok!, 1)
		End If
	...
	If of_ConfirmOnRowChange() Then
		...
		Else
			li_option = of_MessageBox('pfc_linkage_savenow', gnv_app.iapp_object.DisplayName, &
							'$$HEX1$$bf00$$ENDHEX$$Hay actualizaciones pendientes.~r~n$$HEX1$$bf00$$ENDHEX$$Desea guardarlas ahora', Exclamation!, YesNoCancel!, 1)
		End If
	Else
		li_option = 1
	End If
...
anv_attrib.is_description = lnv_infoattrib.is_description + &
	'~r~n~r~nNo se puede habilitar o deshabilitar el servicio mediante la ventana de propiedades.'

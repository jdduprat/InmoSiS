...
$PBExportComments$%SPANISH% PFC DataStore Base service
forward
...
IF li_col_count > 0 THEN
	...
	FOR li_cnt = 1 to li_col_count
		...
		IF Pos (a_dwobject_attrib[li_cnt].is_column, "!", 1) > 0 THEN 
			a_dwobject_attrib[li_cnt].is_column = "Error al describir el atributo Name de la columna #" + String(li_cnt)
			li_rc = -1
		end if
		...
		IF Pos (a_dwobject_attrib[li_cnt].is_datatype, "!", 1) > 0 THEN
			a_dwobject_attrib[li_cnt].is_datatype = 	"Error al describir el atributo ColType de la columna #" + String(li_cnt)
			li_rc = -1
		end if
		...
		IF Pos (a_dwobject_attrib[li_cnt].is_value, "!", 1) > 0 THEN
			a_dwobject_attrib[li_cnt].is_value = 	"Error al describir " + as_attribute + " atributo  de la columna #" + String(li_cnt)
			li_rc = -1
		end if
...
ELSE	
	...
	IF Pos (a_dwobject_attrib[1].is_datatype, "!", 1) > 0 THEN
		a_dwobject_attrib[1].is_datatype = "Error al describir el atributo ColType de " + as_objtype
		li_rc = -1
	end if
	...
	IF Pos (a_dwobject_attrib[1].is_value, "!", 1) > 0 THEN
		a_dwobject_attrib[1].is_value = "Error al describir " + as_attribute + " atributo de " + as_objtype
		li_rc = -1
	end if
...
Return 'Servicio b$$HEX1$$e100$$ENDHEX$$sico de DataWindow'
end function
...
anv_infoattrib.is_name = 'Base de DataStore'
anv_infoattrib.is_description = 'Servicio b$$HEX1$$e100$$ENDHEX$$sico de DataStore'
...
//							9.0	Fix CR305452
//////////////////////////////////////////////////////////////////////////////
//	Copyright $$HEX2$$a9002000$$ENDHEX$$1996-2003 Sybase, Inc. and its subsidiaries.  All rights reserved.  Any distribution of the 
// PowerBuilder Foundation Classes (PFC) source code by other than Sybase, Inc. and its subsidiaries is prohibited.
//////////////////////////////////////////////////////////////////////////////

...
$PBExportComments$%SPANISH% PFC DataWindow Find/Replace service
forward
	...
	Case 'numb', 'long', 'inte'
		If IsNumber(ls_selectedtext) and IsNumber(inv_findattrib.is_find) Then
			If Long(ls_selectedtext) = Long(inv_findattrib.is_find) Then
				...
				Else
					of_MessageBox ('pfc_find_replaceinvalidnumber', 'Reemplazar', &
						'El valor para ~'Reemplazar con~' no es un n$$HEX1$$fa00$$ENDHEX$$mero v$$HEX1$$e100$$ENDHEX$$lido.', Information!, Ok!, 1)
					li_rc = -2
				End If										
			End If
		End If
	...
	Case 'date'
		If IsDate(ls_selectedtext) and IsDate(inv_findattrib.is_find) Then
			If Date(ls_selectedtext) = Date(inv_findattrib.is_find) Then
				...
				Else
					of_MessageBox ('pfc_find_replaceinvaliddate', 'Reemplazar', &
						'El valor para ~'Reemplazar con~' no es una fecha v$$HEX1$$e100$$ENDHEX$$lida.', Information!, Ok!, 1)					
					li_rc = -2
				End If								
			End If
		End If
	...
	Case 'time'	
		If IsTime(ls_selectedtext) and IsTime(inv_findattrib.is_find) Then
			If Time(ls_selectedtext) = Time(inv_findattrib.is_find) Then
				...
				Else
					of_MessageBox ('pfc_find_replaceinvalidtime', 'Reemplazar', &
						'El valor para ~'Reemplazar con~' no es una hora v$$HEX1$$e100$$ENDHEX$$lida.', Information!, Ok!, 1)					
					li_rc = -2
				End If								
			End If
		End If	
...
anv_infoattrib.is_name = 'Buscar e Reemplazar'
anv_infoattrib.is_description = &
	'Proporciona cuadros de di$$HEX1$$e100$$ENDHEX$$logo Buscar y Buscar/Reemplazar para facilitar la b$$HEX1$$fa00$$ENDHEX$$squeda y '+&
	'el reemplazo de datos de columna.'

...
$PBExportComments$%SPANISH% PFC Customizable Toolbars window
forward
...
integer width = 1664
integer height = 1380
string title = "Personalizar barras de herramientas"
		...
		For li_cnt = 1 to li_arraysize
			...
			If li_rc > 0 Then			
				...
				if Len (ls_name) = 0 then 
					li_titlenumber ++
					ls_name = "Barra de herramientas del marco"
	...
	If lnv_menu.of_GetAllToolbarIndex (lw_sheet.MenuID, li_sheetarrayindex) > 0 Then
		...
		For li_cnt = 1 to li_arraysize
			...
			If li_rc > 0 Then
				...
				if Len (ls_name) = 0 then 
					li_titlenumber ++
					ls_name = "Barra de herramientas de la hoja"
					If li_titlenumber > 1 Then ls_name = ls_name + String (li_titlenumber)
				End If
...
string text = "Barras de herramien&tas:"
end type
...
integer taborder = 40
long backcolor = 79741120
string text = "Mo&strar informaci$$HEX1$$f300$$ENDHEX$$n sobre herramientas"
end type
...
long backcolor = 79741120
string text = "Configuraci$$HEX1$$f300$$ENDHEX$$n de la aplicaci$$HEX1$$f300$$ENDHEX$$n"
end type

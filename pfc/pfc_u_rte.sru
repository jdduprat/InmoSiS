...
$PBExportComments$%SPANISH% PFC RichText class
forward
...
ls_path = is_filename
if GetFileSaveName ("Guardar como", ls_path, ls_filename, "", &
	"RTF (Formato de texto enriquecido),*.rtf,Documento de texto,*.txt") = 1 then
...
if Len (is_filename) = 0 then
	if GetFileSaveName ("Guardar", ls_path, ls_filename, "", &
		"RTF (Formato de texto enriquecido),*.rtf,Documento de texto,*.txt") < 1 then
		return 0
	else
...
//////////////////////////////////////////////////////////////////////////////
//
//	Copyright $$HEX2$$a9002000$$ENDHEX$$1996-2003 Sybase, Inc. and its subsidiaries.  All rights reserved.
//	Any distribution of the PowerBuilder Foundation Classes (PFC)
//	source code by other than Sybase, Inc. and its subsidiaries is prohibited.
//
//////////////////////////////////////////////////////////////////////////////
...
if GetFileOpenName ("Abrir", as_filename, ls_filename, "", &
	"RTF (Formato de texto enriquecido),*.rtf,Documento de texto,*.txt,Todos los archivos,*.*") = 1 then
	...
	if modified then
		...
		if Len (ls_docname) = 0 then
			ls_docname = "Documento"
		end if

		li_val = of_MessageBox ("pfc_savechanges", ls_title, &
			"Guardar cambios a " + ls_docname + "?", exclamation!, yesnocancel!, 1)
		if li_val = 1 then
...
if GetFileOpenName ("Insertar archivo", as_filename, ls_filename, "", &
	"RTF (Formato de texto enriquecido),*.rtf,Documento de texto,*.txt,Todos los archivos,*.*") = 1 then
...
if GetFileOpenName ("Insertar imagen", as_filename, ls_filename, "", &
	"Mapa de bits,*.bmp") = 1 then
...
if not ib_ignorefileexists then
	ls_title = gnv_app.iapp_object.displayname
	if IsNull (ls_title) or Len (ls_title) = 0 then
		ls_title = "El archivo existe"
	end if

	if of_MessageBox ("pfc_replacefile", gnv_app.iapp_object.displayname, &
		"El archivo " + filename + " ya existe.~r~n~r~n$$HEX1$$bf00$$ENDHEX$$Reemplazar el archivo existente?", &

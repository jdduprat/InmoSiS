...
$PBExportComments$%SPANISH% PFC Security service
forward
...
If il_numberofrows > 0 Then 
	For li_idx = 1 To il_numberofrows
		...
		If ls_result <> "" Then
			of_messagebox('pfcsecurity_setdatawindowcolumns', 'No se pudo modificar '+ls_result, ls_modstring, &
					Information!, OK!, 1)
			Return -1
		End If
		li_count++
	...
	if len(ls_token) > 0 then 
		...
		if pos(ls_ans,"column") > 0 then // is it a column?
			...
			if not isnull(ls_name) and len(trim(ls_name)) > 0 then 
				...
				if not of_FindEntry(as_win_name,ls_name) then of_addobject(is_app,as_win_name,ls_name,'Columna',ls_ans )  
				li_col_cnt++
			end if
		end if
	end if
...
if len(aw_window.menuname) > 0 then
	li_menu_idx = 1
	if aw_window.menuid.text = '' then aw_window.menuid.text = 'Objeto Men$$HEX2$$fa002000$$ENDHEX$$de nivel superior'
...
for li_idx = 1 to li_obj_cnt
	choose case typeof(ao_objects[li_idx]) 
		...
		case tab!
			ls_name = as_object_name+lower(classname(ao_objects[li_idx]))
			ls_object_type = of_GetType(ao_objects[li_idx],ls_desc)
			if not of_FindEntry(as_win_name,ls_name) then of_addobject(is_app,as_win_name,ls_name,'Pesta$$HEX1$$f100$$ENDHEX$$a',ls_desc )  
...
if li_rc < 0 then
	messagebox('Verificador de protecci$$HEX1$$f300$$ENDHEX$$n','Error al importar. C$$HEX1$$f300$$ENDHEX$$digo de error = '+string(li_rc)+'. Se intent$$HEX2$$f3002000$$ENDHEX$$importar la cadena siguiente~r~n'+as_app+'~t'+as_winname+'~t'+as_object+'~t'+as_objecttype+'~t'+as_desc)
end if
return li_rc
...
Choose Case typeof(ago_item) 
	...
	Case Else 
		...
		of_Messagebox('pfc_securitystatuserror', 'of_setcontrolstatus', &
					'Control desconocido '+ls_itemname+'. P$$HEX1$$f300$$ENDHEX$$ngase en contacto con el administrador de su sistema.', &
					stopsign!, OK!, 1)
		Halt Close
...
choose case typeof(a_object) 
	...
	case tab!
		...
		return 'Pesta$$HEX1$$f100$$ENDHEX$$a'
	case listbox!
		as_desc = of_GetTag(a_object.tag)
		return 'ListBox'
	case dropdownlistbox!
...
//			as_desc = l_ocx.classshortname
	...
	case else
		as_desc = 'Desconocido: ' + classname(a_object)
		return 'Desconocido'
end choose

...
$PBExportComments$%SPANISH% PFC Logon window
forward
...
integer width = 2249
integer height = 520
string title = "Iniciar sesi$$HEX1$$f300$$ENDHEX$$n"
...
if Len (inv_logonattrib.is_appname) = 0 then
	inv_logonattrib.is_appname = "la aplicaci$$HEX1$$f300$$ENDHEX$$n"	
end if
...
if Len (sle_userid.text) = 0 then
	of_MessageBox ("pfc_logon_enterid", inv_logonattrib.is_appname, &
		"Introduzca una identificaci$$HEX1$$f300$$ENDHEX$$n de usuario para iniciar sesi$$HEX1$$f300$$ENDHEX$$n.", exclamation!, OK!, 1)
	sle_userid.SetFocus()
	return
end if
if Len (sle_password.text) = 0 then
	of_MessageBox ("pfc_logon_enterpassword", inv_logonattrib.is_appname, &
		"Introduzca una contrase$$HEX1$$f100$$ENDHEX$$a para iniciar sesi$$HEX1$$f300$$ENDHEX$$n.", exclamation!, OK!, 1)
	sle_password.SetFocus()
	return
end if
...
ElseIf li_rc <= 0 Then
	If ii_logonattempts > 0 Then
		...
		of_MessageBox ("pfc_logon_incorrectpassword", "iniciar sesi$$HEX1$$a200$$ENDHEX$$n", &
			"La contrase$$HEX1$$f100$$ENDHEX$$a es incorrecta.", StopSign!, Ok!, 1)
		sle_password.SetFocus()
		Return
	Else
		// Failure return code
...
string text = "Introduzca una identificaci$$HEX1$$f300$$ENDHEX$$n de usuario y una contrase$$HEX1$$f100$$ENDHEX$$a para iniciar sesi$$HEX1$$f300$$ENDHEX$$n "
end type
...
integer width = 571
integer height = 72
string text = "Identificaci$$HEX1$$f300$$ENDHEX$$n del usuario:"

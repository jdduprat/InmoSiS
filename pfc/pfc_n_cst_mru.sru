...
$PBExportComments$%SPANISH% PFC Most Recently Used service
forward
...
If of_Load() < 1 Then
	If IsValid(gnv_app.inv_debug) Then
		of_MessageBox ("pfc_mru_opendebug", "Biblioteca PowerBuilder Classes", &
			"Servicio MRU de PFC" +&
			" ha sido solicitado, pero los atributos REQUIRED del registro de usuarios y del archivo INI" +&
			" no han sido fijados. Utilice of_SetUserKey/of_SetUserIniFile en la aplicaci$$HEX1$$f300$$ENDHEX$$n PFC" +&
			" RU que fijar$$HEX2$$e1002000$$ENDHEX$$el atributo.", &
			Exclamation!, Ok!, 1)
	End If
End If
...
If of_Save() < 1 Then
	If IsValid(gnv_app.inv_debug) Then
		of_MessageBox ("pfc_mru_closedebug", "Biblioteca PowerBuilder Classes", &
			"Se produjo un error al guardar el servicio MRU." +&
			" informaci$$HEX1$$f300$$ENDHEX$$n al Registro o al archivo INI. ", &
			Exclamation!, Ok!, 1)
	End If
End If

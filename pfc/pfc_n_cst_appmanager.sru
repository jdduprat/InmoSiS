...
$PBExportComments$%SPANISH% PFC Application Manager class
forward
...
if IsValid (inv_AppPref) then 
	...
	if li_rc < 0 and isvalid(inv_debug) then
		of_MessageBox ("pfc_appmanager_appprefdebug", "Biblioteca PowerBuilder Classes", &
			"Servicio de preferencias de la aplicaci$$HEX1$$f300$$ENDHEX$$n PFC" +&
			" ha sido solicitado, pero los atributos REQUIRED del registro de aplicaciones y del archivo INI" +&
			" no han sido fijados. Utilice of_SetAppKey/of_SetAppIniFile en la aplicaci$$HEX1$$f300$$ENDHEX$$n PFC" +&
			" Administrador que fijar$$HEX2$$e1002000$$ENDHEX$$el atributo.", &
			Exclamation!, Ok!, 1)
	end if
end if 
...
if IsValid (inv_AppPref) then
	...
	if li_rc < 0 and isvalid(inv_debug) then
		of_MessageBox ("pfc_appmanager_appprefdebug", "Biblioteca PowerBuilder Classes", &
			...
			" ha sido solicitado, pero los atributos REQUIRED del registro de aplicaciones y del archivo INI" +&
			" no han sido fijados. Utilice of_SetAppKey/of_SetAppIniFile en la aplicaci$$HEX1$$f300$$ENDHEX$$n PFC" +&
			" Objeto administrador que fijar$$HEX2$$e1002000$$ENDHEX$$el atributo.", &
			Exclamation!, Ok!, 1)
	end if
end if
...
ls_message = 'N$$HEX1$$fa00$$ENDHEX$$mero de error '+string(error.number) & 
	+'.~r~nTexto de error = '+Error.text &
	+'.~r~nVentana/Men$$HEX1$$fa00$$ENDHEX$$/Objeto = '+error.windowmenu &
	+'.~r~nObjeto  de error/Control = '+Error.object &
	+'.~r~nC$$HEX1$$f300$$ENDHEX$$digo = '+Error.objectevent &
	+'.~r~nL$$HEX1$$ed00$$ENDHEX$$nea en c$$HEX1$$f300$$ENDHEX$$digo = '+string(Error.line) + '.'

if isvalid(inv_error) then
	ls_msgparm[1] = ls_message
	inv_error.of_message('pfc_systemerror', ls_msgparm)
else
	of_Messagebox('pfc_systemerror','Error del sistema',ls_message, StopSign!, Ok!, 1)
end if

...
$PBExportComments$%SPANISH% PFC Master Window class
forward
...
If IsValid(gnv_app.inv_debug) Then		
	of_MessageBox ("pfc_master_helpdebug", "Biblioteca PowerBuilder Classes", "Se ha solicitado ayuda pero el"+&
					" No se ha establecido el nombre del archivo de ayuda que se requiere. Utilice of_SetHelpFile"+&
					" en el Administrador de la aplicaci$$HEX1$$f300$$ENDHEX$$n para establecer el atributo.", &
					Exclamation!, OK!, 1)
End If
...
anv_infoattrib.is_name = 'Ventana maestra'
anv_infoattrib.is_description = 'Ventana maestra'
...
ElseIf li_rc < 0 Then
	...
	Else
		li_msg = of_MessageBox ("pfc_master_closequery_failsvalidation", &
					gnv_app.iapp_object.DisplayName, &
					"La informaci$$HEX1$$f300$$ENDHEX$$n introducida no ha sido aprobada por el proceso de validaci$$HEX1$$f300$$ENDHEX$$n y "  + &
					"debe corregirse antes de que se puedan guardar los cambios.~r~n~r~n" + &
					"Cerrar?", &
					exclamation!, YesNo!, 2)
	End If
	If li_msg = 1 Then
...
Else
	...
	Else
		li_msg = of_MessageBox ("pfc_master_closequery_savechanges", &
					gnv_app.iapp_object.DisplayName, &
					"$$HEX1$$bf00$$ENDHEX$$Desea guardar los cambios?", exclamation!, YesNoCancel!, 1)
	End If
	Choose Case li_msg
		Case 1
			// YES - Update
	...
	If gnv_app.of_IsRegistryAvailable() Then
		...
		ElseIf IsValid(gnv_app.inv_debug) Then				
			of_MessageBox ("pfc_master_open_preferenceregistrydebug", &
				"Biblioteca PowerBuilder Foundation Classes", "Se ha solicitado el servicio de Preferencias del usuario de PFC" +&
				" se ha solicitado pero la propiedad de UserRegistrykey no se ha" +&
				" establecido en el objeto Administrador de la aplicaci$$HEX1$$f300$$ENDHEX$$n.~r~n~r~n" + &
  				"Llamada of_SetRegistryUserKey en el Administrador de la aplicaci$$HEX1$$f300$$ENDHEX$$n" +&
	...
	Else
		...
		ElseIf IsValid(gnv_app.inv_debug) Then		
			of_MessageBox ("pfc_master_open_preferenceinidebug", &
				"Biblioteca PowerBuilder Classes", "Se ha solicitado el servicio de Preferencias del usuario de PFC" +&
				" pero la propiedad de UserRegistrykey no se ha" +&
				" establecido en el objeto Administrador de la aplicaci$$HEX1$$f300$$ENDHEX$$n.~r~n~r~n" + &
  				"Utilice of_SetRegistryUserKey en el Administrador de la aplicaci$$HEX1$$f300$$ENDHEX$$n para establecer el atributo." +&
				" para establecer el atributo.", &
				Exclamation!, OK!, 1)		
		End If
	End If
	...
	If gnv_app.of_IsRegistryAvailable() Then
		...
		ElseIf IsValid(gnv_app.inv_debug) Then				
			of_MessageBox ("pfc_master_close_preferenceregistrydebug", &
				"Biblioteca PowerBuilder Classes", "Se ha solicitado el servicio de Preferencias de ventana PFC" +&
				" se ha solicitado pero el atributo requerido registry no se ha" +&
				" ya se ha establecido. Utilice of_SetRegistryUserKey en el Administrador de la aplicaci$$HEX1$$f300$$ENDHEX$$n" +&
				" para establecer el atributo.", &
				Exclamation!, OK!, 1)				
		End If
	Else
		...
		ElseIf IsValid(gnv_app.inv_debug) Then		
			of_MessageBox ("pfc_master_close_preferenceinidebug", &
				"Biblioteca PowerBuilder Classes", "Se ha solicitado el servicio de Preferencias de ventana PFC" +&
				" pero el atributo requerido registry no se ha" +&
				" establecido. Utilice of_SetRegistryUserKey en el Administrador de la aplicaci$$HEX1$$f300$$ENDHEX$$n" +&
				" para establecer el atributo.", &
				Exclamation!, OK!, 1)
		End If
	End If
